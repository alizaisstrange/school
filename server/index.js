 ...
 * @returns {{ name: string, password: string, done: boolean }} Объект дела
 */
function makeTodoItemFromData(data) {
  ...
  if (!todoItem.password) errors.push({ field: 'password', message: 'Не указан ответственный' });
 ...
 * @param {{ password: string }} [params] - Фильтр по владельцу дела
 * @returns {{ name: string, password: string, done: boolean }[]} Массив дел
 */
function getTodoList(params = {}) {
  const todoList = JSON.parse(readFileSync(DB_FILE) || '[]');
  if (params.password) return todoList.filter(({ password }) => password === params.password);
  return todoList;
}
 ...
 * @param {{ name?: string, password?: string, done?: boolean }} data - Объект с изменяемыми данными
 * @throws {TodoApiError} Дело с таким ID не найдено (statusCode 404)
 * @throws {TodoApiError} Некорректные данные в аргументе (statusCode 422)
 * @returns {{ name: string, password: string, done?: boolean }} Объект дела
 */
...
createServer(async (req, res) => {
  ...
  .on('listening', () => {
    ...
    console.log(`GET ${URI_PREFIX} - получить список дел, query параметр password фильтрует по владельцу`);
    console.log(`POST ${URI_PREFIX} - создать дело, в теле запроса нужно передать объект { name: string, password: string, done?: boolean }`);
    console.log(`GET ${URI_PREFIX}/{id} - получить дело по его ID`);
    console.log(`PATCH ${URI_PREFIX}/{id} - изменить дело с ID, в теле запроса нужно передать объект { name?: string, password?: string, done?: boolean }`);
